...
./scripts/opt.sh --no-open-diff --action optimize --mode static --input /path/to/skill_dir
```
...
默认会生成 `<skill_dir>/.opt/diff.html` 并在单个 Skill 场景下自动打开；可加 `--no-open-diff` 仅生成不打开。

```bash
# 如需手动重新生成/查看（建议加 --no-open，避免重复弹窗）
uv run python scripts/diff_viewer.py --snapshots /path/to/skill-snapshots --title "skill-name" --no-open --output diff.html
```
