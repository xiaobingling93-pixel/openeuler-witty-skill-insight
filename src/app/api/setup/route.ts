...
function generateBashScript(host: string, baseUrl: string): string {
    const lines = [
        ...
        'if [ "$INSTALL_OPENCODE" = "true" ]; then echo "  ✅ OpenCode Plugin: ~/.opencode/plugins/Skill-Insight.ts"; fi',
...
function generatePowerShellScript(host: string, baseUrl: string): string {
    const lines = [
        ...
        '    Invoke-WebRequest -Uri "$SKILL_INSIGHT_BASE_URL/api/setup/opencode" -OutFile "$homeDir\\.opencode\\plugins\\Skill-Insight.ts"',
        '    ',
        ...
        'if ($INSTALL_OPENCODE) { Write-Host "  ✅ OpenCode Plugin: ~/.opencode/plugins/Skill-Insight.ts" }',
