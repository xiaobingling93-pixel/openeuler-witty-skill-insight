#!/usr/bin/env bash
# run.sh — fault-pattern-security 测试运行脚本
#
# 用法:
#   ./run.sh                    # 使用默认输出目录
...
# 默认输出目录
OUTPUT_DIR="$PROJECT_ROOT/tests/skill-generator/output/fault-pattern-security"
VALIDATE_ONLY=false
VALIDATE_ONLY_PATH=""
...
echo ""
echo "╔══════════════════════════════════════╗"
echo "║  Skill Generator 测试: fault-pattern-security ║"
echo "║  安全故障列表 → 排查 Skill           ║"
echo "╚══════════════════════════════════════╝"
...
PROMPT="使用 skill-generator，基于以下安全故障模式列表生成排查 Skill（--auto 模式，全部采纳推荐选项，无需交互确认），输出目录为 ${OUTPUT_DIR}。
...
# 找到生成的 Skill 目录（可能在 OUTPUT_DIR 下的子目录）
...
if [[ $EXIT_CODE -eq 0 ]]; then
    echo "🎉 fault-pattern-security 测试通过！"
    echo "   生成位置: $OUTPUT_DIR"
else
    echo "💥 fault-pattern-security 测试失败，请检查上方的验证报告。"
fi
echo ""
exit $EXIT_CODE
